...
<div class="panel panel-default policy-page">
    ...
    <div class="panel-body policy-page-body">
        ...
        <ul>
            ...
            <li><strong>订货中商品等待供应商配送（已开始处理）：</strong>不提供自助改址，需在<a href="{{ auth()->check() ? route('support.feedbacks.create') : route('login') }}">客户反馈</a>中提出变更信息申请。</li>
        ...
        <ul>
            <li><strong>订单状态处理中，但等待供应商配送：</strong>此阶段订单内已不提供申请退款按钮，需在<a href="{{ auth()->check() ? route('support.feedbacks.create') : route('login') }}">客户反馈</a>中提出取消订单申请。进入等待配送状态不满 7×24 小时，取消订单收取 20% 取消费用（即 80% 退款）；等待超过 7×24 小时，或本店已确认供应商近期无法正常供货，取消订单免除取消费用（100% 全额退款）。</li>
            <li><strong>订单状态处理中（已开始发货处理）：</strong>若经本店确认包裹还未送往物流仓库，订单可以取消，取消订单收取 20% 取消费用（退款 80%）。此阶段需在<a href="{{ auth()->check() ? route('support.feedbacks.create') : route('login') }}">客户反馈</a>中提出申请。若包裹已送往物流仓库，因需物流人员配合找出对应包裹，无法承诺来得及取消，建议不要抱有太大期待。</li>
